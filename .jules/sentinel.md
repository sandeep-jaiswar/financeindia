...
## 2024-06-29 - [Fix HTTPS Downgrade Vulnerability in HTTP Client]
**Vulnerability:** The HTTP client configured in `src/common.rs` via `reqwest::ClientBuilder` did not strictly enforce HTTPS. This could allow an attacker to intercept or modify traffic via man-in-the-middle (MitM) attacks by downgrading connections to plain HTTP.
**Learning:** Default behavior of HTTP clients often allows unencrypted HTTP connections unless explicitly prevented. We must actively enforce HTTPS, especially when communicating with external financial APIs that may contain sensitive data or have spoofable responses.
**Prevention:** Always configure `reqwest::ClientBuilder` with `.https_only(true)` to ensure that all requests are encrypted. When testing against local mock servers, this protection can be conditionally disabled using an environment variable like `FINANCEINDIA_TEST_ENV`.
