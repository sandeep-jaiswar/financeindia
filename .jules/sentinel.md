...
## 2025-05-24 - [Fix cleartext HTTP requests by enforcing HTTPS]
**Vulnerability:** The HTTP client configured in `src/common.rs` via `reqwest::ClientBuilder` did not strictly enforce HTTPS, which could allow the client to be downgraded or explicitly instructed to communicate over cleartext HTTP. This would expose sensitive data to interception or modification via Man-in-the-Middle (MitM) attacks.
**Learning:** Default HTTP client configurations may allow HTTP connections if not explicitly restricted. When communicating with modern APIs, enforcing HTTPS at the client level provides an additional layer of defense against misconfigurations or malicious redirects.
**Prevention:** Always explicitly configure HTTP clients with `.https_only(true)` (or equivalent settings) to prevent them from sending cleartext requests, conditionally disabling this protection only during local testing.
