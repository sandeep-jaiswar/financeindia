...
## 2024-05-26 - [MitM / Downgrade Attack via Plain HTTP]
**Vulnerability:** The HTTP client configured in `src/common.rs` did not enforce HTTPS. If an attacker could intercept the connection or if a plain HTTP URL was requested (e.g. via redirection or misconfiguration), sensitive data could be transmitted in plaintext, allowing Man-in-the-Middle (MitM) attacks.
**Learning:** Default HTTP client configurations may allow insecure plain HTTP connections. When dealing with sensitive financial data or APIs that should always use HTTPS, this must be explicitly enforced.
**Prevention:** Always enforce HTTPS on the core HTTP client (e.g., using `builder.https_only(true)` for `reqwest`) to prevent accidental or malicious downgrades to plaintext HTTP, ensuring this is bypassed conditionally only for local testing environments.
