...
## 2025-02-16 - Enforce HTTPS Only on Reqwest Client
**Vulnerability:** The `reqwest::Client` in `src/common.rs` was not enforcing HTTPS-only communication. An attacker could potentially intercept or manipulate requests via downgrade attacks or Man-in-the-Middle (MitM) attacks if HTTP were used.
**Learning:** Default configurations might not strictly enforce secure channels. Ensuring all outgoing requests use HTTPS is critical for maintaining data integrity and confidentiality.
**Prevention:** Always configure `reqwest::ClientBuilder` with `.https_only(true)` in production to prevent downgrade attacks and ensure all communication is encrypted. Use environment variables to disable this only for testing purposes (e.g., when communicating with a local HTTP mock server).
