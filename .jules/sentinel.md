...
## 2026-07-29 - [Fix Zip Slip/Path Traversal validation over-blocking absolute paths]
**Vulnerability:** The input validation for output ZIP paths in `archive_equities` rejected `Component::RootDir` and `Component::Prefix(_)`. This was intended to prevent path traversal (Zip Slip equivalents), but incorrectly prevented users from specifying valid absolute paths or drive letters for their output files.
**Learning:** When implementing path traversal protections for output paths provided by users, differentiating between absolute paths (which are generally acceptable for local tooling) and relative directory traversals (e.g., `..`) is crucial. Blocking all absolute paths can break legitimate use cases without adding meaningful security against Zip Slip.
**Prevention:** Only block `Component::ParentDir` (`..`) when validating output directory paths unless the specific requirement strictly mandates relative paths only.
