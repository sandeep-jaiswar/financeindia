...
## 2025-05-24 - [Enforce HTTPS-only in HTTP Client]
**Vulnerability:** The HTTP client in `src/common.rs` was not explicitly configured to restrict requests to HTTPS only. If a developer accidentally hardcoded an `http://` URL, or if an open redirect sent the client to an `http://` endpoint, sensitive data could be transmitted in plaintext, leaving it vulnerable to Man-In-The-Middle (MitM) and downgrade attacks.
**Learning:** Default HTTP clients generally allow both HTTP and HTTPS schemes. Relying solely on URL validation or developer discipline is insufficient.
**Prevention:** Always enforce HTTPS-only connections at the HTTP client level (`reqwest::ClientBuilder::https_only(true)`). Apply this conditionally (e.g., using a test environment variable) so that local tests against mock HTTP servers are not broken.
