...
## 2025-05-25 - Enforce HTTPS Only on Central HTTP Client
**Vulnerability:** The central `reqwest::ClientBuilder` in `src/common.rs` did not explicitly enforce HTTPS connections. If an endpoint URL was accidentally constructed with `http://` instead of `https://` (e.g. by passing an insecure referer or if a redirect returned a plaintext URL), the client would silently establish an unencrypted connection, risking Man-in-the-Middle (MitM) attacks and plaintext data leakage of sensitive financial data requests.
**Learning:** Even if primary API URLs are hardcoded as `https://`, defense in depth dictates that the HTTP client itself should be configured to reject unencrypted traffic outright, mitigating risks from dynamic URLs, redirects, and developer mistakes.
**Prevention:** Always add `.https_only(true)` to `reqwest::ClientBuilder` to strictly enforce TLS and prevent downgrade attacks or accidental plaintext traffic.
