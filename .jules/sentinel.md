...
## 2026-05-27 - [Fix SSRF via HTTP Client Missing https_only]
**Vulnerability:** The HTTP client created in `src/common.rs` via `reqwest::ClientBuilder` did not strictly enforce HTTPS connections. Although custom redirect policies enforced the `https` scheme for redirects, the initial request could potentially be made over plain HTTP, exposing the application to MitM attacks or accidental insecure data transfer.
**Learning:** Default behavior of `reqwest::ClientBuilder` allows both HTTP and HTTPS schemes. We must explicitly opt-in to restricting connections to HTTPS only to guarantee secure communication.
**Prevention:** Always configure `reqwest::ClientBuilder` with `.https_only(true)` (unless explicitly overridden for local test environments using an environment variable like `FINANCEINDIA_TEST_ENV`) to enforce secure connections.
