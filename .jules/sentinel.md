...
## 2025-05-24 - [Missing HTTPS Enforcement in HTTP Client]
**Vulnerability:** The central `reqwest::ClientBuilder` in `src/common.rs` did not enforce `.https_only(true)`. An attacker with a Man-in-the-Middle (MitM) position could potentially intercept and modify requests/responses by forcing a downgrade to plain HTTP, exposing sensitive financial data.
**Learning:** Even if the URLs hardcoded in the application use `https://`, a downgrade attack or an explicit user input that uses `http://` could result in insecure communication if the HTTP client itself does not enforce secure schemes at the client level.
**Prevention:** To enforce secure HTTP connections and prevent downgrade or MitM attacks, configure the central `reqwest::ClientBuilder` with `.https_only(true)`. Apply this conditionally (e.g., using `FINANCEINDIA_TEST_ENV=1`) to avoid breaking local integration test environments that rely on `http://127.0.0.1` for mock servers.
