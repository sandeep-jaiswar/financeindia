...
## 2024-05-26 - [Downgrade / MitM via HTTP]
**Vulnerability:** The centralized `reqwest::Client` in `src/common.rs` was not restricted to HTTPS only, allowing potential downgrade or MitM attacks if HTTP URLs were requested.
**Learning:** PyO3 extensions (`cdylib`) tested via Python do not have `#[cfg(test)]` active during `pytest` runs. Using `#[cfg(not(test))]` to enforce `https_only(true)` would result in tests failing because the compiled module would still strictly enforce HTTPS while tests might use HTTP mock servers.
**Prevention:** Configure the central `reqwest::ClientBuilder` with `.https_only(true)` and use an environment variable (e.g., `FINANCEINDIA_TEST_ENV`) to conditionally disable it during testing, ensuring strict HTTPS in production while allowing local testing.
