...
## 2025-05-24 - [Fix Unencrypted HTTP Connections (MitM) in reqwest Client]
**Vulnerability:** The HTTP client in `src/common.rs` did not enforce `.https_only(true)`. Even with domain whitelists and safe redirects, an attacker performing a Man-in-the-Middle (MitM) attack could intercept HTTP requests (or coerce a downgrade) to read or modify sensitive financial data in transit.
**Learning:** For libraries acting as clients to sensitive third-party APIs, enforcing HTTPS at the client configuration level prevents accidental or coerced unencrypted requests. When the client is built as a Python extension module (`cdylib`), standard Rust `#[cfg(test)]` directives are not active during testing, so environment variables (like `FINANCEINDIA_TEST_ENV`) must be used to bypass security policies for local mock testing.
**Prevention:** Always configure `reqwest::ClientBuilder` with `.https_only(true)` for production builds when sensitive external data is being fetched.
