...
**Vulnerability:** The `build_client` function in `src/common.rs` created a `reqwest::Client` without specifying a custom redirect policy. By default, `reqwest::Client` follows redirects across any domain. This allowed an attacker to bypass domain-based SSRF filters by providing a URL to an attacker-controlled endpoint on an initially trusted domain, which would then redirect the request to an internal or restricted external domain.
**Learning:** Checking the domain string before making the request is insufficient if the HTTP client automatically follows redirects to unchecked domains.
**Prevention:** Always configure an explicit, custom `reqwest::redirect::Policy` that restricts allowed redirect target hosts to a predefined whitelist of trusted domains and limits the redirect count to prevent infinite loops.
...
## 2024-05-26 - [DoS via Trusted Content-Length in HTTP Client]
**Vulnerability:** The HTTP client response fetching logic in `src/common.rs` (`fetch_bytes`) trusted the `Content-Length` header and used `reqwest::Response::bytes()` if it was within `MAX_RESPONSE_SIZE`. An attacker or compromised server could send a valid `Content-Length` but return an infinitely large payload (decompression bomb or raw data stream), circumventing the DoS limit because `bytes()` reads the entire body into memory without intermediate length checks.
**Learning:** Never trust the `Content-Length` header for memory allocation or safety bounds when processing HTTP responses, as it can be easily spoofed or bypassed via HTTP compression (e.g., gzip/brotli bombs).
**Prevention:** Always use streaming (e.g., `reqwest::Response::bytes_stream()`) to read the response body in chunks, strictly enforcing a maximum byte limit (e.g., `MAX_RESPONSE_SIZE`) dynamically during chunk accumulation, and gracefully handling chunk errors to allow the external retry mechanism to engage.
## 2025-05-24 - [Fix Unbounded Memory Allocation (DoS) in HTTP Client]
...
## 2024-05-27 - [Fix SSRF via Open Redirects in Client Builder]
**Vulnerability:** The HTTP client configured in `src/common.rs` used a custom redirect policy but explicitly followed HTTP redirects (`attempt.follow()`) regardless of the URL scheme, which allowed an attacker to redirect an HTTPS request to an HTTP endpoint. It only checked `attempt.url().scheme() == "https"` to verify it.
**Learning:** Checking the URL scheme inside the redirect policy manually via `attempt.url().scheme() == "https"` is not as robust or explicit as using `builder.https_only(true)`, which guarantees the entire client instance blocks downgrades to HTTP across initial requests and redirects. The project convention handles testing without HTTPS by using `FINANCEINDIA_TEST_ENV`.
**Prevention:** In `reqwest`, explicitly configure `builder.https_only(true)` unless testing locally. This enforces HTTPS for all connections, preventing downgrade attacks and eliminating the need for manual scheme validation in custom redirect policies.
