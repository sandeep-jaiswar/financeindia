...
## $(date +%Y-%m-%d) - [Fix Missing HTTPS Enforcement in HTTP Client]
**Vulnerability:** The central `reqwest::ClientBuilder` in `src/common.rs` did not explicitly enforce HTTPS connections. If an initial request or a redirect inadvertently targeted an `http://` URL instead of `https://`, the client would establish a plaintext connection, potentially exposing sensitive data (e.g. cookies or referrers) to Man-In-The-Middle (MitM) attacks.
**Learning:** Default configurations for HTTP clients in Rust do not automatically enforce TLS. In financial/market data libraries, it is critical to explicitly configure clients to reject all unencrypted HTTP connections to prevent accidental downgrades or configuration errors.
**Prevention:** Always enforce HTTPS strictly by configuring `.https_only(true)` on the `reqwest::ClientBuilder` (bypassed conditionally only during local tests via environment variables).
