...
## 2024-05-27 - [Fix SSRF via Default HTTP Client Scheme]
**Vulnerability:** The HTTP client created via `reqwest::ClientBuilder::new()` did not enforce the `https` scheme for outbound requests by default. Since the library handles user-provided or externally fetched data and relies heavily on custom redirect behaviors, an attacker or compromised intermediate node could coerce the client to perform raw HTTP requests, facilitating man-in-the-middle attacks or bypassing certain SSRF protections.
**Learning:** Default configurations in HTTP clients often permit insecure protocols. When writing financial APIs, explicit enforcement of transport-layer security (HTTPS) should be built-in.
**Prevention:** Configure the HTTP client with `.https_only(true)` to strictly prevent any non-HTTPS traffic out of the box. Ensure to conditionally toggle this based on standard environment variables during testing to not break mock suites.
