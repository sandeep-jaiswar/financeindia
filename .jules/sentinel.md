...
## 2025-05-27 - [Enforce HTTPS only in reqwest client]
**Vulnerability:** The HTTP client configured in `src/common.rs` via `reqwest::ClientBuilder` did not strictly enforce HTTPS for all requests. This could allow an attacker to perform Man-in-the-Middle (MitM) attacks or protocol downgrade attacks, potentially intercepting or modifying sensitive financial data in transit.
**Learning:** Even if URLs are hardcoded to use `https://`, redirects or dynamically constructed URLs could inadvertently use `http://`, exposing data.
**Prevention:** Configure the HTTP client to explicitly enforce HTTPS only using `.https_only(true)`, ensuring all requests, regardless of the URL scheme, are securely encrypted.
