...
## 2024-05-27 - [MitM via Unencrypted HTTP Traffic]
**Vulnerability:** The HTTP client configured in `src/common.rs` via `reqwest::ClientBuilder` did not strictly enforce HTTPS for all outbound connections. If an attacker controlled network traffic, or an open redirect pointed to a plain `http://` URL, the client could leak sensitive query parameters or be subjected to a Man-in-the-Middle (MitM) attack.
**Learning:** Default client configurations don't prevent downgrade attacks to plain HTTP. When building clients for secure APIs, you must explicitly enforce secure transport, and account for test environments where HTTPS might not be feasible without custom certificates.
**Prevention:** Configure the HTTP client using `.https_only(true)` to guarantee all connections are encrypted, using environment variables to gracefully disable this restriction during testing.
