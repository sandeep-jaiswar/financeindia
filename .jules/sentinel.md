...
## 2024-05-26 - [DoS via Missing Response Size Validation in HTTP Client]
**Vulnerability:** The HTTP client relied on `reqwest::Response::bytes().await`, which buffers the entire response in memory. While it checked `Content-Length` beforehand, an attacker (or a compromised/maliciously configured server via open redirect) could omit or spoof the `Content-Length` header, leading to excessive memory allocation (OOM) and a Denial of Service (DoS) when reading large responses.
**Learning:** Checking `Content-Length` is insufficient for enforcing maximum payload sizes because it is controlled by the remote server and can be missing or inaccurate. Relying on `bytes()` in these scenarios can lead to unbound memory consumption.
**Prevention:** Always read HTTP responses dynamically using streams (e.g., `bytes_stream()`) and enforce a maximum size limit (`MAX_RESPONSE_SIZE`) on the accumulated bytes during the read loop to fail fast and prevent memory exhaustion.
