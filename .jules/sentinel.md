...
## 2025-05-24 - [Fix Downgrade/MitM Vulnerability in HTTP Client]
**Vulnerability:** The HTTP client configured in `src/common.rs` did not enforce `.https_only(true)`. Although redirects were checked to use the HTTPS scheme, the initial request could potentially be made over HTTP if an attacker managed to replace a URL or if an API endpoint accepted HTTP. This opens the door to Man-in-the-Middle (MitM) attacks or protocol downgrade vulnerabilities where data could be sniffed or altered in transit.
**Learning:** Default configurations in HTTP clients do not strictly enforce HTTPS for all requests, requiring explicit configuration to guarantee secure channels, especially when dealing with financial data.
**Prevention:** Always enforce HTTPS strictly by configuring the `reqwest::ClientBuilder` with `.https_only(true)`. For testing purposes, apply this conditionally using environment variables (e.g., `FINANCEINDIA_TEST_ENV`) rather than relying on `#[cfg(not(test))]`, because `cfg(test)` is not active when building a PyO3 extension (`cdylib`) for Python tests.
