...
## 2026-06-08 - Enforcing HTTPS on HTTP Client with Test Overrides
**Vulnerability:** The HTTP client previously allowed insecure `http://` connections. While public endpoints were hardcoded to `https://`, redirects or future dynamic endpoint changes could inadvertently route traffic over plain HTTP, leaving the system vulnerable to man-in-the-middle (MitM) attacks.
**Learning:** Enforcing HTTPS unconditionally using `reqwest::ClientBuilder.https_only(true)` provides strong security but breaks local integration test environments that rely on `http://127.0.0.1` for mock servers. Security features that break core testability will be rejected.
**Prevention:** Apply client-level security enforcements (like `https_only(true)`) conditionally. Use conditional compilation (e.g., `#[cfg(not(test))]`) to enforce the restriction only in non-test builds, allowing `http` strictly for local testing while securing production.
