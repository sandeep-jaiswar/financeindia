...
## 2024-05-27 - Unenforced HTTPS on HTTP Client
**Vulnerability:** The central `reqwest::ClientBuilder` did not strictly enforce HTTPS via `https_only(true)`. Even though most known URLs use `https://`, untrusted user input or an open redirect could downgrade a connection to `http://`.
**Learning:** Default configurations allow unencrypted HTTP, leaving the application vulnerable to Man-in-the-Middle (MitM) downgrade attacks.
**Prevention:** Always enable `https_only(true)` on HTTP clients, conditionally turning it off only for explicit local test environments (e.g., when `FINANCEINDIA_TEST_ENV` is set).
