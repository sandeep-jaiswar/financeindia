...
## 2025-05-24 - [Enforce HTTPS Only to Prevent Downgrade Attacks]
**Vulnerability:** The HTTP client configured in `src/common.rs` via `reqwest::ClientBuilder` did not strictly enforce HTTPS. This could potentially allow the client to fall back to plain HTTP if a redirect or API endpoint mistakenly (or maliciously via downgrade attack) uses an `http://` scheme, exposing sensitive financial data or API interactions in plaintext.
**Learning:** Default HTTP client behaviors allow for plain HTTP connections unless explicitly configured otherwise. When dealing with APIs that are expected to be secure (like financial data), enforcing HTTPS at the client level is a necessary defense-in-depth measure.
**Prevention:** Always configure `reqwest::ClientBuilder` with `.https_only(true)` for production use to enforce encryption, preventing downgrade attacks and accidental plaintext data transmission. Use environment variables (like `FINANCEINDIA_TEST_ENV`) to conditionally disable this constraint only during local testing if mock servers require it.
