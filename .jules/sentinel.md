...
## 2024-05-26 - [Enforce HTTPS to Prevent MitM/Downgrade Attacks]
**Vulnerability:** The HTTP client configured in `src/common.rs` via `reqwest::ClientBuilder` did not strictly enforce HTTPS for all requests. This allowed a Man-in-the-Middle (MitM) attacker or a malicious redirect to downgrade connections to plaintext HTTP, potentially exposing sensitive data, session tokens, or allowing request manipulation.
**Learning:** Even if the initial request URLs or redirect policies validate the scheme, relying on default client behavior allows the possibility of a downgrade attack if a developer inadvertently makes a request using HTTP, or if a redirect target is somehow compromised.
**Prevention:** Always explicitly enforce HTTPS-only connections at the client level using `reqwest::ClientBuilder::https_only(true)` to ensure that all HTTP requests, regardless of how they are constructed, are encrypted and secure against downgrade attacks.
