...
## 2025-05-24 - [Enforce HTTPS connections for core HTTP client]
**Vulnerability:** The central `reqwest::Client` used for all API requests was not strictly configured to enforce HTTPS-only connections. This could potentially allow downgrade attacks or unencrypted HTTP connections if an endpoint URL was misconfigured or if an attacker intercepted traffic.
**Learning:** Default configurations in HTTP clients often allow both HTTP and HTTPS. In financial data applications, ensuring all outbound connections are encrypted is critical.
**Prevention:** Always explicitly configure the HTTP client to enforce HTTPS (e.g., using `.https_only(true)` in `reqwest`) in production environments to prevent accidental unencrypted data transmission.
