...
## 2026-06-12 - [Enforce HTTPS in HTTP Client]
**Vulnerability:** The HTTP client configured in `src/common.rs` did not strictly enforce HTTPS connections. Although URLs are generally defined as HTTPS, a redirect or dynamic input could potentially downgrade a connection to plain HTTP, exposing data to Man-in-the-Middle (MitM) attacks.
**Learning:** Relying solely on hardcoded `https://` URLs is insufficient if the HTTP client is allowed to follow redirects to `http://` endpoints or if dynamic URL inputs are accepted.
**Prevention:** Always configure `reqwest::ClientBuilder` with `.https_only(true)` to enforce secure connections at the transport layer, conditionally disabling it via an environment variable only for testing purposes.
