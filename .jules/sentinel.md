...
## 2026-06-17 - [Enforce HTTPS Only in HTTP Client]
**Vulnerability:** The HTTP client configured in `src/common.rs` via `reqwest::ClientBuilder` allowed unencrypted HTTP connections. This exposed the client to potential downgrade attacks and Man-in-the-Middle (MitM) attacks, allowing an attacker to intercept and modify sensitive financial data.
**Learning:** Even if the initial request URLs are HTTPS, redirects or dynamic URLs could potentially use unencrypted HTTP.
**Prevention:** Always enforce `.https_only(true)` on the HTTP client builder to guarantee that all connections, including redirects, use secure HTTPS channels. Use environment variables (e.g., `FINANCEINDIA_TEST_ENV`) to disable this check only during local or integration testing.
