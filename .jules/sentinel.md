...
## 2024-05-24 - Enforce HTTPS Only to Prevent MitM
**Vulnerability:** The central `reqwest::ClientBuilder` in `src/common.rs` did not enforce `.https_only(true)`, allowing potential downgrade attacks or Man-in-the-Middle (MitM) if any endpoint used `http://` instead of `https://`.
**Learning:** In PyO3 extension modules (`cdylib`), `#[cfg(test)]` directives are only active during `cargo test`. They do not apply when the module is built via `cargo build` and tested via Python's `pytest`. This makes it challenging to toggle security checks for local testing versus production.
**Prevention:** Enforce HTTPS globally with `.https_only(true)` and use environment variables like `FINANCEINDIA_TEST_ENV` to conditionally bypass these security restrictions during testing, ensuring local mock servers can still function without breaking real-world security.
