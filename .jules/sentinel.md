...
## 2024-07-16 - Prevent Downgrade/MitM attacks by enforcing HTTPS
**Vulnerability:** The centralized HTTP client built via `reqwest::ClientBuilder::new()` in `src/common.rs` allowed HTTP schemes by default, creating a risk of accidental unencrypted transmission or active downgrade attacks (MitM) if any endpoint was misconfigured or targeted.
**Learning:** While the custom redirect policy strictly validated redirects to HTTPS, the initial request scheme lacked any systemic enforcement, leaving the client vulnerable.
**Prevention:** Always configure `reqwest::ClientBuilder` with `.https_only(true)` (except when testing against local unencrypted mock servers) to guarantee that the application exclusively communicates over secure encrypted channels.
