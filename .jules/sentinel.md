...
## 2026-05-27 - [Enforce HTTPS only in central HTTP client]
**Vulnerability:** The central HTTP client created via `reqwest::ClientBuilder` in `src/common.rs` did not enforce HTTPS connections. If an attacker managed to redirect traffic or if a URL was inadvertently set to HTTP instead of HTTPS, the client could make unencrypted connections, exposing sensitive financial data or allowing man-in-the-middle (MitM) attacks.
**Learning:** Security defaults in generic HTTP clients might not be strict enough for libraries dealing with sensitive API interactions. We must explicitly enforce secure channels.
**Prevention:** Always enable `https_only(true)` on `reqwest::ClientBuilder` when fetching sensitive data to prevent downgrade attacks or accidental unencrypted communication. Use environment variables (e.g. `FINANCEINDIA_TEST_ENV`) to selectively bypass this restriction only during local testing.
