...
## 2024-06-15 - Enforce HTTPS Connections in HTTP Client
**Vulnerability:** The centralized HTTP client built via `reqwest::ClientBuilder::new()` in `src/common.rs` did not enforce HTTPS. This could allow an attacker to intercept or modify traffic through a Man-in-the-Middle (MitM) attack by forcing a downgrade to plain HTTP.
**Learning:** Even if the library primarily accesses `https://` URLs, an explicit `.https_only(true)` enforcement ensures that all subsequent requests (including redirects) and accidentally hardcoded plain `http://` URLs cannot expose traffic to MitM attacks.
**Prevention:** Always enforce `.https_only(true)` on `reqwest::ClientBuilder` for production clients. Make this conditional based on a test environment variable (e.g. `FINANCEINDIA_TEST_ENV`) to still allow local HTTP testing.
